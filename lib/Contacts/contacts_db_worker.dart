...
class ContactsDBWorker implements EntryDBWorker<Contact> {
  ...
  Future<int> create(Contact contact) async {
    ...
    return await db.rawInsert(
        'INSERT INTO $TBL_NAME ($KEY_NAME, $KEY_PHONE, $KEY_EMAIL, $KEY_BIRTHDAY) '
            'VALUES (?, ?, ?, ?)',
        [contact.name, contact.phone, contact.email, contact.birthdayInUnix ?? -1]
    );
  }
  ...
  Contact _contactFromMap(Map<String, dynamic> map) => Contact(
      id: map[KEY_ID],
      name: map[KEY_NAME],
      phone: map[KEY_PHONE],
      email: map[KEY_EMAIL])
    ..birthdayInUnix = map[KEY_BIRTHDAY] < 0 ? null : map[KEY_BIRTHDAY];

  Map<String, dynamic> _contactToMap(Contact contact) => <String, dynamic>{}
    ..[KEY_ID] = contact.id
    ..[KEY_NAME] = contact.name
    ..[KEY_PHONE] = contact.phone
    ..[KEY_EMAIL] = contact.email
    ..[KEY_BIRTHDAY] = contact.birthdayInUnix ?? -1;
}
